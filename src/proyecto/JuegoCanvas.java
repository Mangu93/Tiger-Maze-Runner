...
public class JuegoCanvas extends Canvas3D {
    ...
    public void postRender() {
        ...
        if (juego.totem1_eliminado) {
            gd.drawString("Has cogido la primera venus", POSICION_VIDA_VERTICAL, POSICION_VIDA_HORIZONTAL * 6);
        }
        if (juego.totem2_eliminado) {

            gd.drawString("Ya tienes las dos venus, ve a por el jefe", POSICION_VIDA_VERTICAL, POSICION_VIDA_HORIZONTAL * 7);
        }
        if (juego.personaje.muerto) {
            gd.drawString("Game Over ", POSICION_VIDA_VERTICAL, POSICION_VIDA_HORIZONTAL * 8);
        }
        if (juego.boss_is_dead) {
            gd.drawString("Victoria, has derrotado al jefe final ", POSICION_VIDA_VERTICAL, POSICION_VIDA_HORIZONTAL * 8);
